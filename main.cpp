...
public:
    ...
    vector< BoundingBox > Detect (const cv::Mat& img, const COLOR_ORDER color_order, const IMAGE_DIRECTION orient, int min_size = 120, float P_thres = 0.6, float R_thres = 0.7, float O_thres =0.7, bool is_fast_resize = true, float scale_factor = 0.709);
...
int main(int argc, char **argv) {
    ...
    //FaceDetector fd("/home/rcnn/projects/FaceAlignment/MTCNN_C/model_v2", FaceDetector::MODEL_V1);
    
	FaceDetector fd("/app/huang/MTCNN/model/MTCNNv1/model", FaceDetector::MODEL_V1);
...
*/
int main(){
	cv::VideoCapture cap("rtsp://admin:gdjy2015@10.0.6.168:554/Streaming/Channels/101");
	FaceDetector fd("/app/huang/MTCNN/model/MTCNNv1/model", FaceDetector::MODEL_V1);
	double fps=cap.get(CV_CAP_PROP_FPS);
	cout<<fps<<endl;
	//cv::Mat testImg = cv::imread("/app/huang/MTCNN/test1.jpg");
	cv::Mat frame;
	if (!cap.isOpened()){
	...
	while (true) {
		...
		vector<FaceDetector::BoundingBox> res = fd.Detect(frame, FaceDetector::BGR, FaceDetector::ORIENT_UP, 60, 0.7, 0.7, 0.7);
		cout << "detected face NUM : " << res.size() << endl;
		if (res.size() != 0) {
			cv::Mat crop;
			for (int k = 0; k < res.size(); k++)
			{
				double height = res[k].y2 - res[k].y1;
				double width = res[k].x2 - res[k].x1;
				//cout << res[k].x1 << " " << res[k].y1 << " " << res[k].x2 << " " << res[k].y2 << endl;
				cout <<"height= "<< height << ", width= " << width << endl;

				frame(cv::Rect(res[k].x1, res[k].y1, height, width)).copyTo(crop);
				baiduAPI(crop);
				//cv::rectangle(frame, cv::Point(res[k].x1, res[k].y1), cv::Point(res[k].x2, res[k].y2), cv::Scalar(0, 255, 255), 2);
				//for (int i = 0; i < 5; i++)
					//cv::circle(frame, cv::Point(res[k].points_x[i], res[k].points_y[i]), 2, cv::Scalar(0, 255, 255), 2);
			}
			auto t = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
			struct tm * ptm = localtime(&t);
			char date[128];
			sprintf(date, "%02d-%02d-%02d %02d-%02d-%02d", (int)ptm->tm_year + 1900, (int)ptm->tm_mon + 1, (int)ptm->tm_mday, (int)ptm->tm_hour, (int)ptm->tm_min, (int)ptm->tm_sec);
			//cv::imwrite("captured_" + string(date) + ".jpg", crop);
			count++;
